...
''' Has 3 dropdowns - width, length, and height - price is just calculated by multiplying the 3
''' </summary>
''' <remarks></remarks>
Public Class SampleCustomControl2
...
#Region "REQUIRED METHODS" 'The methods inside this region must be always be implemented to ensure compatibility with Kartris Custom Product Control code
    ...
    Public Overrides Function CalculatePrice(ParameterValues As String) As Double
        ...
        Try
            ...
            'Width, Length and Height for this custom control so we're expecting 3 indexes
            _CalculatedPrice = arrParameters(0) * arrParameters(1) * arrParameters(2)
    ...
    Public Overrides Function ComputeFromSelectedOptions() As String
        Try
            ...
            _ItemDescription = ddlWidth.SelectedValue & " x " & ddlLength.SelectedValue & " x " & ddlHeight.SelectedValue & " item"

            'Prepare the comma separated list of parameter values based on the selected options in the control
            _ParameterValues = ddlWidth.SelectedValue & "," & ddlLength.SelectedValue & "," & ddlHeight.SelectedValue

            'Formula to compute the item price - this varies from one custom product control to another
            _ItemPrice = ddlLength.SelectedValue * ddlWidth.SelectedValue * ddlHeight.SelectedValue
...
#Region "Custom Code" 'This region of code will vary from one custom product control to another. Developer has freedom to add methods here.
    ''' <summary>
    ''' Handle SelectedIndexChanged event for all dropdowns - width, length and height
    ''' </summary>
    ''' <param name="sender"></param>
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Protected Sub AnyDropdown_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlLength.SelectedIndexChanged, ddlWidth.SelectedIndexChanged, ddlHeight.SelectedIndexChanged
