...
<script type="text/javascript" language="javascript">
    ...
    function ShowLargeViewPopup() {
        $find('cntMain_UC_ProductView_tbcProduct_tabMain_fvwProduct_UC_PopUpLargeView_popMessage').show();
    }

    //Function to set URL for iframe of media popup, size it, and show it
    function ShowMediaPopup(ML_ID, MT_Extension, intParentID, strParentType, intWidth, intHeight) {
        //Set some variables we use later
        var objFrame = $find('cntMain_UC_ProductView_tbcProduct_tabMain_fvwProduct_UC_PopUpMedia_popMessage');

        var objMediaIframeBaseUrl = document.getElementById('media_iframe_base_url').value;
        var objPopupWindow = document.getElementById('cntMain_UC_ProductView_tbcProduct_tabMain_fvwProduct_UC_PopUpMedia_pnlMessage');
    ...
    <asp:UpdatePanel ID="updProduct" runat="server" UpdateMode="Conditional">
        <ContentTemplate>
            <ajaxToolkit:TabContainer CssClass=".tab" ID="tbcProduct" runat="server" OnDemand="false" OnClientActiveTabChanged="" AutoPostBack="false"  EnableTheming="False">
                ...
                VERSIONS/OPTIONS
                --%>
                        <user:ProductVersions ID="UC_ProductVersions" runat="server" EnableViewState="true" />
